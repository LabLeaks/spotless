...
**Date**: 2026-02-26
**Context**: First implementation of agent identity (identity_nodes + 4 digest tools + selector seeding). Live-tested with Haiku digesting. This document catalogs issues found and proposes revisions.

---

## Live Test Results

Seeded an agent with 18 raw events simulating a multi-turn interaction including user correction ("don't over-engineer"), trust building, and autonomous delegation. Ran two digest passes.

**What the digester did:**
...
1. Haiku reads third-person transcripts ("USER said X, ASSISTANT said Y") and is asked to develop first-person self-knowledge. Who is "I"? The digest agent isn't the agent that had the conversation. It's reading someone else's diary and being asked to introspect.
...
Split identity maintenance out of the general digest loop. Make it a **separate pipeline stage** that runs deterministically after the consolidation pass, not inside it.

**Architecture:**

```
Digest pass (existing):
  1. Consolidation loop (Haiku, 16 tools, multi-turn)
     → factual memories, associations, corrections, pruning
  2. Reflection pass (Haiku, SEPARATE invocation, identity-only tools)
...
- The reflection pass sees consolidated memories, not raw events — reflecting on structured knowledge, not transcripts
...
The digest prompt says "You are a memory consolidation system." The identity tools ask it to record self-insights. But it's not reflecting on *itself* — it's reading transcripts about a different Claude instance. The result: relationship model is written in third person ("User trusts agent"), not first person ("They trust me to make scope decisions now").

This matters because when the selector injects identity nodes into the main agent's context, third-person framings feel like dossier entries, not self-knowledge. "Agent tends to over-engineer" is information *about* someone. "I tend to over-engineer" is a constraint on *how I act*.

### Proposed Fix

The **reflection pass** (see above) should use first-person framing:
...
### M1: Dead `roleLabels` variable (selector-prompt.ts:44)

```typescript
const roleLabels: Record<string, string> = { core: "Core", self: "Self", relationship: "Relationship" };
```

Declared, never used. The WORKING SELF section shows `[id:N] content` with no role labels. The selector can't tell which identity node is which (core vs self vs relationship). The plan's example showed labeled output.
...
**Recommendation:** Label them. The selector needs to know "this is the self-model" vs "this is the project summary" to use them correctly. Query identity_nodes for role mapping, render as `[Core, id:N]`, `[Self, id:N]`, `[Relationship, id:N]`.

### M2: Identity nodes appear twice in selector prompt
...
`evolveCoreSummary` transfers memory_sources from old → new. `evolveIdentity` and `evolveRelationship` don't. Inconsistency — old source links are lost, digester can't trace provenance.
...
Live test produced "no such column: engineer" error. The digester passed a raw query string to `query_memories` that wasn't properly sanitized for FTS5 syntax. The `queryMemories` function passes the query directly to FTS5 MATCH without sanitization (unlike `recall()` which uses `sanitizeFts5Query`).
...
### M7: Doc header in digest-tools.ts is stale
...
Every self-insight gets the same salience regardless of importance. "I tend to over-engineer" and "I used bun for testing" would both be 0.85. By contrast, `create_memory` lets the digester choose salience.

**Question:** Should `reflectOnSelf` accept a salience parameter? The plan's rationale is the self-reference effect (SRE) — self-relevant encoding produces uniformly stronger memories. The strength comes from connectivity (association to self-model), not salience variation. Fixed salience seems intentional. Keep it, but the reflection pass prompt should be selective about what qualifies as a self-insight.

### A4: Working self seeds recall on every turn unconditionally

"Always include working self nodes if present" in the selector prompt. Even for "what's 2+2?" the selector gets identity nodes. The DMN analogy suggests this is correct (resting state), but DMN *deactivates* during focused external tasks. Our version never deactivates.

**Assessment:** This is probably fine for now. The cost is 1-3 extra memories in the selector prompt (~100 tokens). The benefit is that identity context is always available. If it becomes a token budget issue, we can add a relevance check. Don't optimize prematurely.

### A5: Digest prompt is getting long (7.6k chars system prompt)
...
**Mitigation:** The reflection pass proposal (see Critical section) addresses this by splitting identity tools into a separate, shorter prompt. The consolidation prompt drops back to 12 tools.

### A6: The brain-region labels in the prompt (aPCu, Insula, vmPFC)

These are in the digest prompt as section headers for the identity consolidation goals. Haiku doesn't know what these mean. They're for us, not for the LLM.
...
The digester reads transcripts of a different Claude instance. When it writes "I tend to over-engineer," who is "I"? The answer should be: the *agent* whose memory this is. The agent named "wren" or "identity-test." The digester is a process that maintains the agent's self-knowledge, not a separate entity with its own identity.

The reflection pass framing (see Critical section) makes this clearer: "You are reviewing YOUR memories to update YOUR self-understanding." The digester becomes the agent's internal voice, not a third party.
...
2. **M2: Deduplicate identity nodes in selector prompt** — DONE. `preComputedRecall` filtered to exclude `identityIds`. Test verifies.
...
6. **M1: Wire up role labels in selector prompt** — DONE. `roleLabels` used in template: `[Core, id:N]`, `[Self, id:N]`, `[Relationship, id:N]`.
7. **M7: Doc header** — DONE. Says "16 functions" (correct).

### Architecture revision — DONE:
8. **Separate reflection pass** — DONE. Two-phase digesting: consolidation (12 tools) → reflection (6 tools, first-person framing). `runToolLoop()` shared helper with phase-specific `allowedTools`. Reflection pass triggers deterministically when new memories exist or identity nodes are incomplete.
9. **First-person prompt rewrite** — DONE. Reflection pass uses agent-named first-person framing ("I am {name}, an LLM coding agent"). Agent-experiential memories ("I recommended X and it blew up") not user-dossier ("User prefers Y"). `<your memories>` tag, `WHO YOU ARE` identity section.

---

## DECISION

The current implementation is mechanically sound (204 tests pass, live digest pass works) but architecturally fragile in two ways:
...
The six mechanical bugs should be fixed now. The architecture revision (separate reflection pass) should be the next major piece of work.
