...
- Sliding Tier 2 budget: identity + world-fact memories share a single `TIER2_BUDGET` (12K) pool with `IDENTITY_FLOOR` (2K) and `MEMORY_FLOOR` (2K) minimums. `computeTier2Allocation()` computes sliding split based on actual token needs. Identity facts flow through selector for contextual selection — tagged `[identity]` in candidates so selector can prioritize but also omit irrelevant ones. Proxy partitions selector result IDs into identity vs world-fact bins, allocates budgets, uses `buildIdentitySuffix()` and `buildMemorySuffix()`. Falls back to agent name on cold start. Old `IDENTITY_BUDGET`/`MEMORY_SUFFIX_BUDGET` constants removed.
- Dynamic history budget: history trace budget computed per-request as `CONTEXT_BUDGET (120K) - systemTokens - toolsTokens - TIER2_BUDGET (12K) - SUFFIX_OVERHEAD (1K)`, floored at 20K. With typical CC overhead (~15K system + ~30K tools), history gets ~62K. Net arithmetic unchanged from previous 4K+8K split. Consolidation pressure uses the same dynamic budget (passed through `buildHistoryTrace` → `getConsolidationPressure`). `HISTORY_BUDGET` (144K) kept as default fallback only.
